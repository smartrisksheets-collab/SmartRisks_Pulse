import api from './api';
import type { DashboardData, ExecInsight, UnifiedBrief } from '../types/dashboard';
...
export async function generateUnifiedBrief(): Promise<UnifiedBrief> {
  const res = await api.post<{ data: UnifiedBrief }>('/api/v1/dashboard/unified-brief');
  return res.data.data;
}
